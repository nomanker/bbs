...
$(function (){
        $.ajax({
            url:"../public/operate.php?act=recycle_list",
            method:"get",
            datatype:"json",
            success:function(data){
                ...
                var recycle_list = $("#recycle");
                var recycles = result.data;
                for( var i = 0 , j = recycles.length ; i < j ; i++) {
                    var recycle_dom = get_recycle(recycles[i]);
                    recycle_list.append(recycle_dom);
                }
            },
            error:function(data){
                alert('获取帖子列表失败！');
        ...
        function get_recycle(data){
                    ...
                    //操作按钮
                    var delButton = $('<a title="恢复" href="javascript:;" style="text-decoration:none;">恢复&nbsp;</a>');
                    delButton.attr("title",data['title']);
                    delButton.click(restore);

                    var downButton = $('<a title="清除" href="javascript:;" style="text-decoration:none;">&nbsp;清除</a>');
                    downButton.attr("title",data['title']);
                    downButton.click(recycle_del);
        ...
        //清除
        function recycle_del(title){
            var title = $(this).attr("title");
            var meButton = $(this);
            $.ajax({
                url:"../public/operate.php?act=recycle",
                method:"post",
                data: {
                    title:title,
                },
        ...
        function restore(title){
            ...
            var meButton = $(this);
            $.ajax({
                url:"../public/operate.php?act=recycle_back",
                method:"post",
                data: {
                    title:title,
                },
